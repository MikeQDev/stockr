	...
	public static Double[] getPrice(String s) {
		try {
			BufferedReader reader = new BufferedReader(
					new InputStreamReader(new URL(BASE_URL + s).openConnection().getInputStream()));
			String sx;
			...
			while ((sx = reader.readLine()) != null) {
				...
				if (sx.startsWith("})();</script></head><body><div id=\"app\">")) {
					try {
						start = sx.substring(sx.indexOf("<span class=\"Fw(b) D(ib) Fz(36px) Mb(-4px)"));
					} catch (StringIndexOutOfBoundsException ex) {
						//ex.printStackTrace();
						System.out.println("tried parsing string with length of -->"
								+ sx.substring(sx.indexOf("<span class=\"Fw(b) D(ib) Fz(36px) Mb(-4px)")).length());
						return new Double[] { -1337.37, -1337.37 };
					}
					...
					String startPosNeg = start
							.substring(start.indexOf("<span class=\"Fw(500) D(ib) Pstart(10px) Fz(24px) C($data"));
